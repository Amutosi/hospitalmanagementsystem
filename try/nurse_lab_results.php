...
// Get ALL lab tests (including those the nurse has already acted on)
$lab_tests = $conn->query("
    SELECT plt.*, 
           CONCAT(p.FirstName, ' ', p.LastName) as PatientName,
           p.ContactNo, p.DateOfBirth, p.BloodGroup,
           lt.TestName, lt.NormalRange, lt.StandardCost,
           (SELECT COUNT(*) FROM Prescription WHERE PatientID = p.PatientID AND NurseID = $nurse_id AND LabTestID = plt.PatientLabTestID) as has_nurse_prescription_this,
           (SELECT COUNT(*) FROM PatientTreatment WHERE PatientID = p.PatientID AND PrescribedByNurse = $nurse_id AND LabTestID = plt.PatientLabTestID) as has_nurse_treatment_this,
           (SELECT COUNT(*) FROM Prescription WHERE PatientID = p.PatientID AND DoctorID IS NOT NULL) as has_any_doctor_prescription,
           (SELECT COUNT(*) FROM PatientTreatment WHERE PatientID = p.PatientID AND PrescribedByDoctor IS NOT NULL) as has_any_doctor_treatment
...
<head>
    ...
    <title>Lab Results - Nurse Portal</title>
    ...
    overflow-x: hidden;
        ...
        .page-header h1 { font-size: 28px; font-weight: 700; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
        ...
        .btn-back { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; border: none; padding: 8px 20px; border-radius: 50px; text-decoration: none; }
        ...
        .btn-treatment { background: linear-gradient(135deg, #17a2b8 0%, #667eea 100%); color: white; border: none; padding: 10px 20px; border-radius: 50px; font-weight: 600; }
    </style>
</head>
<body>
    <div class="container-fluid">
        <div class="page-header">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    ...
                    <p class="text-muted mb-0">Review lab results and prescribe treatments/medications (Nurse)</p>
                </div>
                <a href="nurse_dashboard.php" class="btn-back"><i class="bi bi-arrow-left"></i> Back to Dashboard</a>
            </div>
        </div>
        ...
        <div class="filter-buttons">
            ...
            <button class="filter-btn" data-filter="locked">🔒 Doctor Actions</button>
        </div>
        ...
        <div class="row" id="labResultsContainer">
            <?php if($lab_tests && $lab_tests->num_rows > 0): ?>
                <?php while($row = $lab_tests->fetch_assoc()): 
                    ...
                    $hasNurseActedThis = ($row['has_nurse_prescription_this'] > 0 || $row['has_nurse_treatment_this'] > 0);
                    $hasAnyDoctorAction = ($row['has_any_doctor_prescription'] > 0 || $row['has_any_doctor_treatment'] > 0);
                    
                    // Determine card status and action availability
                    if($hasAnyDoctorAction) {
                        $cardClass = 'locked';
                        $statusText = '🔒 Locked - A doctor has already acted on this patient';
                        $canAct = false;
                    } elseif($hasNurseActedThis) {
                        $cardClass = 'acted';
        ...
        function openActionModal(labResult) {
            if(!labResult.CanAct) {
                Swal.fire({ 
                    ...
                    text: labResult.IsLocked ? 'This patient has already been acted upon by a Doctor. Nurses cannot prescribe or assign treatments for this patient.' : 'You have already acted on this lab result.',
            ...
            $('#modalBody').html(`
                <div class="row"><div class="col-md-6"><div class="card mb-3"><div class="card-body"><h6><i class="bi bi-person"></i> Patient Information</h6><hr><p><strong>Name:</strong> ${labResult.PatientName}</p><p><strong>Contact:</strong> ${labResult.ContactNo || 'N/A'}</p><p><strong>DOB:</strong> ${labResult.DateOfBirth || 'N/A'}</p><p><strong>Blood Group:</strong> ${labResult.BloodGroup || 'N/A'}</p></div></div></div>
            ...
            $('#prescriptionForm').off('submit').on('submit', function(e) {
                e.preventDefault();
                $.ajax({ url: 'nurse_prescribe_from_lab.php', type: 'POST', data: $(this).serialize(), success: function(r) { if(r.success) { Swal.fire({ icon: 'success', title: 'Prescription Added!', text: r.message, timer: 2000, showConfirmButton: false }).then(() => { $('#actionModal').modal('hide'); location.reload(); }); } else { Swal.fire({ icon: 'error', title: 'Cannot Prescribe', text: r.message }); } }, error: function() { Swal.fire({ icon: 'error', title: 'Error', text: 'Failed to add prescription' }); } });
            });
            
            $('#treatmentForm').off('submit').on('submit', function(e) {
                e.preventDefault();
                $.ajax({ url: 'nurse_assign_treatment.php', type: 'POST', data: $(this).serialize(), success: function(r) { if(r.success) { Swal.fire({ icon: 'success', title: 'Treatment Assigned!', text: r.message, timer: 2000, showConfirmButton: false }).then(() => { $('#actionModal').modal('hide'); location.reload(); }); } else { Swal.fire({ icon: 'error', title: 'Cannot Assign Treatment', text: r.message }); } }, error: function() { Swal.fire({ icon: 'error', title: 'Error', text: 'Failed to assign treatment' }); } });
            });
        }
    </script>
</body>
